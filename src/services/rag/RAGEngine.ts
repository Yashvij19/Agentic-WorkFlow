...
export class RAGEngine{
  ...
  async execute(params:{
    ...
    const shouldRunVector = plan.strategies.includes('vector') || plan.strategies.includes('hybrid');
    const shouldRunKeyword = plan.strategies.includes('keyword') || plan.strategies.includes('hybrid');

       const [vectorCandidates, keywordCandidates]=await Promise.all([
        shouldRunVector?this.vectorStore.search(orgId , analysis.normalizedQuery , plan.topK,plan.minScore ,plan.metadataFilters ):Promise.resolve([]),
        shouldRunKeyword?this.vectorStore.searchKeyword(orgId , analysis.originalQuery , plan.topK , plan.metadataFilters):Promise.resolve([])
       ]);
